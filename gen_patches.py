...
def get_rand_patch(img, mask,total,sz=32):
    """
    ...
    random_transformation = np.random.randint(1,8)#(1,8)
...
def get_patches(x_dict, y_dict, n_patches, sz=32):
    x = list()
    y = list()
    random.seed(0)
    total_patches = 0
    while total_patches < n_patches:
        ...
        img = x_dict[img_id]
        mask = y_dict[img_id]
        img_patch, mask_patch = get_rand_patch(img, mask,total_patches, sz)
        x.append(img_patch)
        y.append(mask_patch)
        total_patches += 1
